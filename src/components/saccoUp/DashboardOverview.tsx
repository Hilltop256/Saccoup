...
const DashboardOverview: React.FC<DashboardOverviewProps> = ({ onNavigate }) => {
  ...
  const activeLoansList = loans.filter((l: any) => 
    l.status === 'pending' || l.status === 'approved' || l.status === 'treasurer_approved'
  );
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {members.map(m => {
                  ...
                  return (
                    <div key={m.id} className={`flex flex-col items-center p-3 rounded-xl border transition-all ${
                      isPaid ? 'bg-emerald-50 border-emerald-100' : isFailed ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-100'
                    }`}>
                      <div className="relative">
                        <img 
