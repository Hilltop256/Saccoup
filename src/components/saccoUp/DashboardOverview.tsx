...
const DashboardOverview: React.FC<DashboardOverviewProps> = ({ onNavigate }) => {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [statsRes, contribRes, loansRes, membersRes] = await Promise.allSettled([
      ...
      if (contribRes.status === 'fulfilled' && contribRes.value?.contributions) {
        setContributions(
          contribRes.value.contributions.map((c: any) => ({
            id: c.id,
      ...
      if (loansRes.status === 'fulfilled' && loansRes.value?.loans) {
        setLoans(
          loansRes.value.loans.map((l: any) => ({
            id: l.id,
  ...
  const activeLoansList = loans.filter(
    (l: any) => l.status === 'pending' || l.status === 'approved' || l.status === 'treasurer_approved',
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
          ...
          <p className="text-2xl font-bold text-gray-900 mt-1">
            {formatUGX((stats?.total_savings || 0) + roscaTotals.totalSavings)}
          </p>
        </div>
        <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">Per-member tracking</p>
            </div>
            <button onClick={() => onNavigate('rosca')} className="text-xs text-[#0066CC] font-bold hover:underline">
              Manage Draw
            </button>
          </div>
          ...
          {members.length > 0 ? (
            <div className="space-y-6">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {members.map(m => {
                  const status = memberStatusMap[String(m.member_id)] || 'pending';
                  ...
                  return (
                    ...
                    >
                      <div className="relative">
                        <img
                          ...
                          className={`w-10 h-10 rounded-full border-2 ${
                            isPaid ? 'border-emerald-500' : 'border-white'
                          }`}
                          alt=""
                        />
                        {isPaid && (
                      ...
                      <span
                        className={`text-[9px] uppercase font-black mt-1 ${
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => onNavigate('loans')} className="text-sm text-[#0066CC] font-medium hover:underline">
              View All
            </button>
          </div>
          ...
          {activeLoansList.length > 0 ? (
            <div className="space-y-3">
              {activeLoansList.slice(0, 4).map((loan: any) => (
                <div
                  key={loan.id}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => onNavigate('contributions')} className="text-sm text-[#0066CC] font-medium hover:underline">
            All History
          </button>
        </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {contributions.slice(0, 5).map((c: any) => (
                <tr key={c.id} className="text-sm">
                  ...
                  <td className="py-4">
                    <span
                      className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${getStatusColor(c.status)}`}
                    >
                      {c.status}
                    </span>
                  </td>
