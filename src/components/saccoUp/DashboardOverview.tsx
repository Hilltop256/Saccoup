  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* ROSCA-focused summary for ROSCA/Hybrid groups, traditional for SACCO */}
        {isRoscaType ? (
          <>
            {/* Next Draw */}
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
              ...
              {loading ? (
                <div className="h-24 flex items-center justify-center">
                  <svg className="w-6 h-6 animate-spin text-purple-500" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
                </div>
              ) : (
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <div className="relative w-full h-3 bg-gray-100 rounded-full overflow-hidden">
                  <div className="absolute h-full bg-gradient-to-r from-emerald-400 to-emerald-500" style={{ width: `${totalMembers > 0 ? (confirmedCount / totalMembers) * 100 : 0}%` }} />
                  <div className="absolute h-full bg-gradient-to-r from-amber-400 to-amber-500" style={{ left: `${totalMembers > 0 ? (confirmedCount / totalMembers) * 100 : 0}%`, width: `${totalMembers > 0 ? (pendingCount / totalMembers) * 100 : 0}%` }} />
                </div>
                <p className="text-xs text-gray-500 mt-2 text-center">{Math.round(collectionRate)}% collected</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
              ...
              {roscaTotals.totalWinners > 0 ? (
                <div className="space-y-2">
                  {(loans as any[]).filter((l: any) => l.status === 'won').slice(0, 3).map((loan: any, idx: number) => (
                    <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                      <div className="flex items-center gap-2">
                        <span className="text-lg">🏆</span>
                        <span className="text-sm font-medium">{loan.member_name}</span>
                      </div>
                      <span className="text-sm font-bold text-emerald-600">{formatUGX(loan.amount)}</span>
                    </div>
                  ))}
                  {(!loans as any[]).filter((l: any) => l.status === 'won').length === 0 && (
                    <p className="text-sm text-gray-400 text-center py-4">No winners yet</p>
                  )}
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center h-32 text-gray-400">
                  <svg className="w-10 h-10 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-1.5c0-.621-.504-1.125-1.125-1.125h-4.5c-.621 0-1.125.504-1.125 1.125v1.5m0 0H5.25c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h13.5c.621 0 1.125-.504 1.125-1.125V6.75m-9 0V3m9 0V1.5" />
                  </svg>
                  <p className="text-sm font-medium">No winners yet</p>
                  <p className="text-xs mt-1">Draws haven't started</p>
                </div>
              )}
            </div>
          </>
        ) : (
          /* Traditional SACCO Financial Summary */
          <>
            <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm p-6">
              ...
              {loading ? (
                ...
                <>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                    ...
                    <div className="text-center p-3 rounded-xl bg-red-50">
                      ...
                      <p className="text-base font-bold text-red-600">{formatUGX(stats?.total_loans_outstanding || 0)}</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div>
                ...
                <div className="h-48 flex flex-col items-center justify-center text-gray-400">
                  ...
                  <p className="text-xs mt-1">Start recording contributions</p>
                </div>
              )}
            </div>
          </>
