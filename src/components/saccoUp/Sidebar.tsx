...
export type DashboardPage = 'overview' | 'members' | 'contributions' | 'loans' | 'reports' | 'groups' | 'chat' | 'settings' | 'announcements';
...
const navItems: { id: DashboardPage; label: string; icon: string }[] = [
...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, onNavigate, isOpen, onClose, onLogout }) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full w-72 bg-white border-r border-gray-200 z-50 transform transition-transform duration-300 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-bold text-gray-900">Sacco<span className="text-[#0066CC]">Up</span></span>
            </div>
          ...
          <div className="px-4 py-4 border-b border-gray-100">
            <label className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-2 block">Active Group</label>
            {groups.length > 0 ? (
              <>
                <select
                  value={selectedGroup?.id || ''}
                  onChange={(e) => setSelectedGroupId(e.target.value)}
                  className="w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#0066CC] focus:border-transparent outline-none"
                >
                  {groups.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                </select>
                <div className="mt-2 flex items-center gap-2">
                  <span className="text-xs text-gray-500">{selectedGroup?.members_count || 0} members</span>
                  <span className="text-xs text-gray-300">|</span>
                  <span className="text-xs font-medium text-[#00CC99]">{formatUGX(selectedGroup?.total_savings || 0)}</span>
                </div>
              </>
            ) : (
              <p className="text-xs text-gray-400 italic">No groups yet. Create one!</p>
            )}
          </div>
            ...
            <div className="space-y-1">
              {navItems.map((item) => (
                <button key={item.id} onClick={() => { onNavigate(item.id); onClose(); }}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${currentPage === item.id ? 'bg-[#0066CC]/10 text-[#0066CC]' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}>
